...
start();
        ...
        let endAverange = document.querySelector('.averange');

        let index = 0;
        let points = 0;

        document.querySelector('#index').innerText = index+1;

        localStorage.setItem("score", 0);
        localStorage.setItem("counter", 0);
        localStorage.setItem("averange", 0);
...
next.addEventListener('click', function () {
   ...
   counter++;
   localStorage.counter = Number(localStorage.counter) + Number(counter);
   localStorage.score = Number(localStorage.score) + Number(points);
   localStorage.averange = Number(localStorage.score) / Number(localStorage.counter);

    counter = 0;
    endPoints.innerHTML = points;
    endAverange.innerHTML= localStorage.averange;
}
 })
